...
export function createTreeGeometry(kind, level = 0) {
  ...
  const branch = (a, b, radius, color) => {
    ...
    const g = new THREE.CylinderGeometry(radius * .45, radius, delta.length(), level ? 4 : 7);
  ...
  if (pine) {
    ...
    for (let i = 0; i < tiers; i++) {
      ...
      for (let j = 0; j < arms; j++) {
        ...
        add(new THREE.ConeGeometry(.39 * (1-t*.6), .65 * (1-t*.45), level ? 4 : 6), (i+j)%2 ? 0x4f7458 : 0x345641, x*.7, y+.12, z*.7, 1, .7, 1);
      }
    }
    add(new THREE.ConeGeometry(.22,.65,6), 0x577c5a, .04,3.72,0);
  } else {
    ...
    const n = level === 2 ? 4 : level === 1 ? 7 : 12;
    for (let i = 0; i < n; i++) {
      const angle = i * 2.39996, radius = i === 0 ? 0 : .65 + (i % 3) * .17;
      const x = Math.cos(angle) * radius, z = Math.sin(angle) * radius;
      const y = dry ? 2.65 + (i % 3) * .16 : 2.45 + (i % 4) * .32;
      if (level === 0) branch([0, 1.1 + (i % 3) * .24, 0], [x, y, z], .065, bark);
      add(new THREE.IcosahedronGeometry(1, level === 0 ? 1 : 0), palette[i % 3], x, y, z, birch ? .52 : .74, dry ? .30 : birch ? .83 : .66, birch ? .52 : .72);
    }
  }
